...
class matcifar(data.Dataset):
    ...
    def __init__(self, imdb, train, d, medicinal):
        ...
        self.train = train  # training set or test set  up:true
        self.imdb = imdb
        self.d = d    #up:3
        self.x1 = np.argwhere(self.imdb['set'] == 1)
        self.x2 = np.argwhere(self.imdb['set'] == 3)
        self.x1 = self.x1.flatten()#flatten函数讲多维的数组折叠成一维
...
def flip(data):
    ...
    third = first#((y_1,y_2,y_1),axis=1)
...
def load_data(image_file, label_file):
    ...
    data = data_all.reshape(np.prod(data_all.shape[:2]), np.prod(data_all.shape[2:]))  # 512*217=111104(111104,204)prod是将元素中所有相乘
    data_scaler = preprocessing.scale(data)  # (X-X_mean)/X_std,计算方差
...
class ClassBalancedSampler(Sampler):
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            batch = [[i+j*self.num_inst for i in torch.randperm(self.num_inst)[:self.num_per_class]] for j in range(self.num_cl)]#返回分好类的[[],[],[]...]数组
        else:
            batch = [[i+j*self.num_inst for i in range(self.num_inst)[:self.num_per_class]] for j in range(self.num_cl)]#data[x]里面x是位置，[x]里面x是具体的数
