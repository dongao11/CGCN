...
parser.add_argument("-b","--query_num_per_class",type = int, default = 19)#19*9=171
...
def get_train_test_loader(Data_Band_Scaler, GroundTruth, partition):
    ...
    imdb = {}
    imdb['data'] = np.zeros([2 * HalfWidth + 1, 2 * HalfWidth + 1, nBand, nTrain + nTest], dtype=np.float32)  # (9,9,100,n)=>(9,9,103通道,42776)
    imdb['Labels'] = np.zeros([nTrain + nTest], dtype=np.int64)#(42776)0
    imdb['set'] = np.zeros([nTrain + nTest], dtype=np.int64)#(42776)

    RandPerm = train_indices + test_indices

    RandPerm = np.array(RandPerm)#42776   [36730  1284  8004 ... 39890 26418 22109]
    ...
    for iSample in range(nTrain + nTest):
        ...
        imdb['Labels'][iSample] = G[Row[RandPerm[iSample]], Column[RandPerm[iSample]]].astype(np.int64)#(42776,)
    ...
    for iSample in range(da_nTrain):
        ...
        imdb_da_train['Labels'][iSample] = G[Row[da_da_RandPerm[iSample]], Column[da_da_RandPerm[iSample]]].astype(
            np.int64)
