...
def get_train_test_loader(Data_Band_Scaler, GroundTruth, partition):
    ...
    for iSample in range(nTrain + nTest+val_nTrain):
        imdbgraph['data'][:, :, :, iSample] = data[Row[RandPermgraph[iSample]] - HalfWidth:  Row[RandPermgraph[iSample]] + HalfWidth + 1,
            Column[RandPermgraph[iSample]] - HalfWidth: Column[RandPermgraph[iSample]] + HalfWidth + 1, :]
    ...
    imdbgraph['set'] = np.hstack((3 * np.ones([nTrain]), 3 * np.ones([nTest]),3 * np.ones([val_nTrain]) )).astype(np.int64)  # 将两个数组按水平方向组合起来
    test_datasetgraph = utils2.matcifar(imdbgraph, train=False, d=3, medicinal=0)
    test_loader = torch.utils.data.DataLoader(test_datasetgraph, batch_size=9, shuffle=False, num_workers=0)
    ...
    for iSample in range(val_nTrain):
        imdb_val_train['data'][:, :, :, iSample] = data[Row[da_RandPerm[iSample]] - HalfWidth:  Row[da_RandPerm[iSample]] + HalfWidth + 1,
    ...
    imdb_val_train['set'] = np.ones([da_nTrain]).astype(np.int64)
    ...
    for iSample in range(da_nTrain):
        imdb_da_train['data'][:,:,:,iSample]=utils2.radiation_noise(data[Row[da_da_RandPerm[iSample]] - HalfWidth : Row[da_da_RandPerm[iSample]] + HalfWidth + 1,
    ...
    imdb_da_train_labels=imdb_da_train['Labels']-1
