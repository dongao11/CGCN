...
def gmul(input):
    ...
    output = output.split(N, 1)#->N*(bs,j,num_features)
...
class Gconv(nn.Module):
    ...
    def forward(self, input):
        W = input[0]
        x = gmul(input)  # ->output has size (bs, N, J*num_features)-》（bs，n，num_inputs）
        #if self.J == 1:
        #    x = torch.abs(x)
        x_size = x.size()
        x = x.contiguous()
        x = x.view(-1, self.num_inputs) # has size (bs*N, num_inputs)
        x =self.fc(x)#p42-》(bs*N,num_outputs)


        if self.bn_bool:
            x = self.bn(x)#batchnormalization

        x = x.view(*x_size[:-1], self.num_outputs)#（bs，n，num）
        return W, x
...
class Wcompute(nn.Module):#
    ...
    def forward(self, x, W_id):
        W1 = x.unsqueeze(2)
        #print(f"the w1 of gnniclr is {W1.shape}")
        W2 = torch.transpose(W1, 1, 2) #size: bs x N x N x num_features

        W_new = torch.abs(W1 - W2)
        W_new = torch.transpose(W_new, 1, 3) #10，181，6，6

        W_new = self.conv2d_1(W_new)#(w_new,nf)
...
class GNN_nl(nn.Module):
    def __init__(self, args, input_features, nf, J):
        ...
        for i in range(self.num_layers):
            if i == 0:
                module_w = Wcompute(self.input_features, nf, operator='J2', activation='softmax', ratio=[2, 2, 1, 1])#求节点间相似度
            ...
            self.add_module('layer_w{}'.format(i), module_w)#layer-w{}.format就是module_w
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            x_new = F.leaky_relu(self._modules['layer_l{}'.format(i)]([Wi, x])[1])#这里应该是代入（[wi,x]）之后取返回的[1]维
        ...
        return out[:, 0:(self.args.query_set), :],out[:, (self.args.query_set): ,:]
